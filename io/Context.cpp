...
void addWorldToContext(Context& ctx, core::World* world)
{
    ...
    const std::vector<core::ComponentType*> sts = world->getComponentTypes();
    std::for_each(sts.cbegin(), sts.cend(), [&](core::ComponentType* o) { ctx.add(o); });

    const std::vector<core::EntityType*> tts = world->getEntityTypes();
    std::for_each(tts.cbegin(), tts.cend(), [&](core::EntityType* o) { ctx.add(o); });
}
